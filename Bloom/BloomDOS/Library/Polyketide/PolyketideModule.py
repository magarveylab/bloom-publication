...
from BearLite.Chemistry.Reaction import ReactionSmarts
...
class PolyketideModule:
    ...
    def load_module_reactions(self):
        ...
        for row in pd.read_csv(mod_rxn_fp).to_dict('records'):
            ...
            row['only_consider_units'] = set(ast.literal_eval(row['only_consider_units']))
    ...
    def compute_module_monomers(self):
        ...
        for rxn in self.mod_rxns:
            for s in rxn['consider_substrates']:
                # notation
                description = '{}-{}'.format(self.substrates[s]['description'],
                                             rxn['reaction_name'])
                ...
                self.module_monomers.append({
                    ...
                    'substrate_family': self.substrates[s]['substrate_family'],
    ...
    def load_specialized_motifs(self):
        ...
        for row in pd.read_csv(spec_rxn_fp).to_dict('records'):
            rid = row['reaction_id']
            row['enums'] = set(ast.literal_eval(row['enums'])) | {'pk_specialized'}
            row['reaction_smarts'] = ReactionSmarts(row['reaction_smarts'])
            row['predecessor_reaction_ids'] = ast.literal_eval(row['predecessor_reaction_ids'])
            row['predecessor_substrate_ids'] = ast.literal_eval(row['predecessor_substrate_ids'])
            row['ignore_substrate_ids'] = ast.literal_eval(row['ignore_substrate_ids'])
            row['linkers'] = []
            row['bonds'] = []
        ...
        spec_linker_fp = '{}/specialized_motif_linker_descriptors.csv'.format(pk_dir)
        for row in pd.read_csv(spec_linker_fp).to_dict('records'):
            rid = row['reaction_id']
            row['linker_smarts'] = Chem.MolFromSmarts(row['linker_smarts'])
            row['linker_atoms'] = [int(a) for a in ast.literal_eval(row['linker_atoms'])]
        ...
        spec_bond_fp = '{}/specialized_motif_bond_descriptors.csv'.format(pk_dir)
        for row in pd.read_csv(spec_bond_fp).to_dict('records'):
            rid = row['reaction_id']
            row['smarts'] = Chem.MolFromSmarts(row['smarts'])
            row['bonds'] = [[int(x) for x in b.split('-')] for b in ast.literal_eval(row['bonds'])]
    ...
    def load_standalone_motifs(self):
        ...
        for row in pd.read_csv(stand_fp).to_dict('records'):
            ...
            row['linker_atom_enums'] = [ast.literal_eval(i) for i in row['linker_atom_enums'].split(' | ')]
