from BearLite.Library.Polyketide.PolyketideModule import PolyketideModule
from BearLite.Chemistry.SmartsCompiler import get_smarts
from BearLite.Chemistry.Reaction import dethiolation
from BearLite.Chemistry.ChemUtils import(
    get_unmapped_atom_idx,
...
def get_variable_map(smiles: str, linkers: List[dict] = []) -> Dict[int, Set[str]]:
    variable_map = {}
...
def get_alpha_restriction_map(smiles: str) -> List[int]:
    ...
    mol = Chem.MolFromSmiles(smiles)
    return [atom.GetIdx() for atom in mol.GetAtoms() if atom.GetIsotope() == 13]
...
class Polyketide:
    ...
    def add_starter(self):
        for s in tqdm(self.pk_module.starters, desc='Generating polyketide starters'):
            ...
            atoms_to_ignore = [a.GetIdx() for a in mol.GetAtoms() if a.GetAtomMapNum() == 1]
            [a.SetAtomMapNum(0) for a in mol.GetAtoms()]
            product = Chem.MolToSmiles(mol, canonical=False)
            # prepare smarts
            smarts = get_smarts(product,
                                do_H_queries=True,
            ...
            # cache
            self.library.append({'notation': s['notation'],
    ...
    def add_extender(self):
        ...
        for m in tqdm(self.pk_module.module_monomers, desc='Generating polyketide extenders'):
            ...
            alpha_restriction_map = get_alpha_restriction_map(product)
            # cache
            self.library.append({'notation': m['notation'],
    ...
    def add_terminal(self):
        for m in tqdm(self.pk_module.module_monomers, desc='Generating polyketide terminal'):
            for t in self.pk_module.terminal_rxns:
                ...
                description = m['description'] + ' | {}'.format(t['reaction_id'])
                enums = m['enums'] | {'TE'}
                if t['reaction_id'] == 'TE':
                ...
                variable_map = get_variable_map(product)
                if t['reaction_id'] == 'TE':
                    variable_map[terminal_var_atom].extend(['miscellaneous', 'nonmodular'])
                alpha_restriction_map = get_alpha_restriction_map(product)
                # cache
                self.library.append({'notation': notation,
                                     'description': description,
                                     'smarts': smarts,
                                     'smiles': product,
                                     'ignore_patt_idx': list(variable_map.keys()),
    ...
    def add_specialized(self):
        for s in tqdm(self.pk_module.specialized_motifs, desc='Generating polyketide specialized units'):
            for m in self.pk_module.module_monomers:
                if len(s['ignore_substrate_ids']) > 0 and m['substrate'] in s['ignore_substrate_ids']: continue
                if len(s['predecessor_reaction_ids']) > 0 and m['reaction_id'] not in s['predecessor_reaction_ids']: continue
                if len(s['predecessor_substrate_ids']) > 0 and m['substrate'] not in s['predecessor_substrate_ids']: continue
                notation  = m['notation'] + ' | {}'.format(s['reaction_name'])
                description  = m['description'] + ' | {}'.format(s['reaction_name'])
    ...
    def add_specialized_terminal(self):
        for s in tqdm(self.pk_module.specialized_motifs, desc='Generating polyketide specialized terminal units'):
            if s['consider_terminal'] == False: continue
            for m in self.pk_module.module_monomers:
                if len(s['ignore_substrate_ids']) > 0 and m['substrate'] in s['ignore_substrate_ids']: continue
                if len(s['predecessor_reaction_ids']) > 0 and m['reaction_id'] not in s['predecessor_reaction_ids']: continue
                if len(s['predecessor_substrate_ids']) > 0 and m['substrate'] not in s['predecessor_substrate_ids']: continue
                # base notation
                base_notation  = m['notation'] + ' | {}'.format(s['reaction_name'])
                base_description  = m['description'] + ' | {}'.format(s['reaction_name'])
                # calculate product
                ...
                for t in self.pk_module.terminal_rxns:
                    ...
                    description = base_description + ' | {}'.format(t['reaction_id'])
                    # run reaction
    ...
    def add_standalone_motifs(self):
        for s in tqdm(self.pk_module.standalone_motifs, desc='Generating polyketide standalone units'):
            smiles = get_natural_smiles(s['smiles'])
            mol = Chem.MolFromSmiles(smiles)
            map2idx = {a.GetAtomMapNum(): a.GetIdx() for a in mol.GetAtoms() if a.GetAtomMapNum() > 0}
            [a.SetAtomMapNum(0) for a in mol.GetAtoms()]
            product = Chem.MolToSmiles(mol, canonical=False)
            variable_map = {map2idx[x]: y for x, y in zip(s['linker_atoms'], s['linker_atom_enums'])}
            smarts = get_smarts(product,
                                do_H_queries=True,
                                do_hybridization_queries=True,
                                do_heteroatom_n_queries=True,
